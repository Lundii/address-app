 ...
 */
  ...
  export const getSearchResults = async (addressData, updateAddressData) => {
    ...
    const { docs, numFound } = addresses.response;
    const totalPages = Math.ceil(numFound / itemsPerPage);
    if (docs && !docs.length) {
      return updateAddressData(prevState => ({...prevState, error: 'No result found' }));
    }
    updateAddressData(prevState => ({...prevState, totalPages, queryResult: {...addresses.response}}));
  }
